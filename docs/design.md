...
- **{colors.brand-blue}** is the *functional* accent — reserved for "this is interactive or worth noting," never for brand decoration:
  - Inline links and "read more" affordances (case studies, blog detail, legal pages)
  - Checkmarks and emphasis icons inside content lists
  - `focus-visible` outlines on links and form inputs
  - Blockquote / callout left borders
  - It also carries the **Legal** service-pillar's category color and the **Products** nav/footer category color — an intentional reuse of the one functional-blue token rather than a fifth accent color

- **Never**: a solid blue button background (no `button-blue` variant exists here — blue does not fill CTAs, unlike Miro's own system), and never yellow on a link, checkmark, or focus ring — that is blue's job.
